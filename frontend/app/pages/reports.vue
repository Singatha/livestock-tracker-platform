  ...
  <section>
    ...
    <template v-else-if="summary">
      ...
      <div class="mt-6 grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        ...
        <div class="grid gap-6"><Card><CardHeader><CardTitle>Feed inventory value</CardTitle><CardDescription>Quantity multiplied by recorded unit cost.</CardDescription></CardHeader><CardContent><p class="font-heading text-4xl font-bold">{{ Number(summary.inventory_value).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</p><p class="mt-2 text-xs text-muted-foreground">Currency follows your farm’s entered costs.</p></CardContent></Card><Card><CardHeader><CardTitle>Export CSV</CardTitle><CardDescription>Download farm-scoped records for analysis or safekeeping.</CardDescription></CardHeader><CardContent class="grid grid-cols-2 gap-2"><Button v-for="type in ['animals', 'health', 'tasks', 'feed', 'weights', 'medicine', 'reproduction', 'treatment-courses', 'audit']" :key="type" variant="outline" :disabled="!!downloading" class="capitalize" @click="exportReport(type)"><Download /> {{ downloading === type ? 'Downloading…' : type.replace('-', ' ') }}</Button></CardContent></Card></div>
      </div>
    </template>
  </section>
</template>
